module Y2017.M07.D20.Solution where
...
runLogic Success = guard (1 == 1)
runLogic Fail    = guard (1 == 0)

{--
Inspired by Greg Weidman's C++-code:

#define true 1 == 1
#define false 1 == 0

Because you never can tell with C.
--}
...
-- note that fail here is equivalent to Prelude.fail "some error message"
...
runEvalExpr = reduce . map runLogic

reduce :: MonadPlus m => [m ()] -> m ()
reduce = foldr (>>) success
