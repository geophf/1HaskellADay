module Y2017.M02.D17.Solution where
...
-- just like the crossP function from Data.Probability

matchP :: Prob Traits -> Prob Traits -> Prob Traits
matchP = condense . join <<- liftM2 match

grandsons :: (Prob Traits, Prob Traits) -> Prob Traits
grandsons = uncurry matchP

-- What is the probability of atLeast 1 grandson is "Aa Bb" just like dear old
-- Mom and Dad?

justlikemom :: Prob Traits -> Float
justlikemom dist = moregenerally dist 4 1

{--
>>> let son = match parentA parentB
>>> let gs = grandsons (son, return parentA)
>>> justlikemom gs
5.859375e-2

BLEH! Rosalind says the answer should be 0.684. What am I doing wrong here? :/
--}
...
moregenerally :: Prob Traits -> Integer -> Integer -> Float
moregenerally dist n k = fromRational (atLeast dist parentA k n)
