...
module Y2020.M12.D16.Solution where
...
>>> getGraphResponse url [allianceNamesQuery fpda]
...
-- (also note that a row is an array) ... (but we don't care for parsing an unit)

instance FromJSON AllianceNames where
   parseJSON = withObject "Alliance Names" $ \v ->
                  AN <$> v .: "name" <*> aliasesParser v

aliasesParser :: Object -> Parser [Name]
aliasesParser v = fromMaybe [] <$> v .:? "aliases"
...
instance FromJSON Country where
   parseJSON = withObject "Country" $ \v ->
      Country <$> v .: "name" <*> v .:? "qid" <*> aliasesParser v
