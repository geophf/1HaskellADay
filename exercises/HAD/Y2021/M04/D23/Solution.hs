...
module Y2021.M04.D23.Solution where
...
readSkill (despace -> fsfe) = flip withKind fsfe
...
-- we want to use the kind to set the context

withKind :: String -> String -> Maybe Skill
withKind "Fighting Style" fstyl = FS <$> readMaybe fstyl
withKind "Food Eaten" foodage = FE <$> readMaybe foodage

despace :: String -> String
despace = concat . words

{--
>>> mapMaybe (uncurry readSkill) warrior 
[FS KravMaga,FE Barley,FE Rye,FE Milk,FS MuayThai]
--}
